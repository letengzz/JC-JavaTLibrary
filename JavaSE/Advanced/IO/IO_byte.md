...
在本章的第一节《[Java流的概念](http://c.biancheng.net/view/1119.html)》中就提到 InputStream 是 [Java](http://c.biancheng.net/java/) 所有字节输入流类的父类，OutputStream 是 Java 所有字节输出流类的父类，它们都是一个抽象类，因此继承它们的子类要重新定义父类中的抽象方法。
