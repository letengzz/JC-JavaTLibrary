...
例如，用Word 2013保存一个Word文件，这个文件也可以在Word 2016上打开。但反过来，用Word 2016保存一个Word文件，就无法使用Word 2013打开。但是，用Word 2016也可以保存一个格式为Word 2013的文件，这样保存的Word文件就可以用低版本的Word 2013打开，但前提是保存时必须明确指定文件格式兼容Word 2013。
...
​	**例**：使用了lambda表达式的源码版本至少要为8才能编译，使用了`var`关键字的源码版本至少要为10才能编译，使用`switch`表达式的源码版本至少要为12才能编译，且12和13版本需要启用`--enable-preview`参数。

## 查看class版本

1. 通过带有16进制查看方式的文本编辑器打开生成的class文件(使用十六进制打开)。

   ![image-20221018152721184](https://cdn.jsdelivr.net/gh/letengzz/Two-C@main/img/Java/202210181534096.png)

2. 只看第一行数据(前面8个字节是固定的，随后的4个字节是次版本号，再后面的四个字节就是JDK的版本号。

   ![image-20221018152801259](https://cdn.jsdelivr.net/gh/letengzz/Two-C@main/img/Java/202210181534521.png)
